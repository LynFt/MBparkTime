    ...
    def reconnect(self):
        try:
            ...
            if isinstance(self, RabbitComsumer):
                durable = MQ_CONFIG.get("durable", False)
                autoDelete = MQ_CONFIG.get("autoDelete", False)
                
                result = self.channel.queue_declare(durable=True, queue="queue_{0}".format(self.recv_serverid),
                                                    exclusive=False)
                ...
                self.channel.basic_consume(queue=queue_name, on_message_callback=self.consumer_callback, auto_ack=False)
...
class RabbitComsumer(RabbitMQServer):
    ...
    def __init__(self, other_callback=None, **kwargs):
        super(RabbitComsumer, self).__init__()
        self.callback = other_callback
        self.callback_dict = kwargs
    ...
    def consumer_callback(self, ch, method, properties, body):
        ...
        if not self.callback is None:
            self.callback(body, **self.callback_dict)
    ...
    @classmethod
    def run(cls, recv_serverid, other_callback=None, **kwargs):
        consumer = cls(other_callback, **kwargs)
