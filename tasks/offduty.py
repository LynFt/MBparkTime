...
def off_duty(**kwargs):
    ...
    user_sql = AttendPlanMapper.GET_PLAN_BY_PERIOD.format(**kwargs)
    ...
    for group_user in group_users:
        ...
        if group_records is None:
            ...
            if is_hoilday:
                ...
                attend_sql = AttendMapper.ADD_ATTEND.format()
                result = mysql_tools.execute_sql(attend_sql)
            else:
                # 设置为缺勤
                attend_params["status_id"] = 66
                attend_params["type_id"] = 0
                attend_sql = AttendMapper.ADD_ATTEND.format()
                result = mysql_tools.execute_sql(attend_sql)
        else:
            group_record = group_records[-1]
            attend_params = {
                "attends_time": group_record.get("record_time"),
                "attends_user_id": group_record.get("user_id"),
                "attend_hmtime": now_time,
            }
            # 生成打卡状态
            if group_record.get() < now_time:
                ...
                attend_params["type_id"] = 0
                attend_sql = AttendMapper.ADD_ATTEND.format(**attend_params)
                result = mysql_tools.execute_sql(attend_sql)
            else:
                # 异常
                process_sql = ProcessMapper.GET_PROCESS_BY_START_TIME.format(start_time=kwargs.get("start_time"),
